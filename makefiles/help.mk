...
help:
	...
	@printf "### Beacon Tools (Thor backend)\n"
	$(call help_line,tools up,Start Beacon Tools backend services)
	$(call help_line,tools down,Stop Beacon Tools backend services)
	$(call help_line,tools build,Build Beacon Tools backend images)
	$(call help_line,tools logs,Logs for Beacon Tools backend)
	$(call help_line,tools status,Status for Beacon Tools backend)
	$(call help_line,tools-db-init,Apply DB init scripts to beacon-tools-postgres)
	$(call help_line,vectordb-image-rag-init,Apply Image RAG schema to beacon-vectordb)
	@printf "### Thor (Beacon Tools backend)\n"
	$(call help_line,thor up,Bring up Thor docker compose)
	$(call help_line,thor down,Bring down Thor docker compose)
	$(call help_line,thor logs,Logs for Thor services)
	$(call help_line,thor status,Status for Thor services)
	$(call help_line,thor build,Build Thor images)
	@printf "### Frey (Beacon Tools frontend)\n"
	$(call help_line,frey up,Bring up Frey docker compose)
	$(call help_line,frey down,Bring down Frey docker compose)
	$(call help_line,frey logs,Logs for Frey frontend)
	$(call help_line,frey status,Status for Frey frontend)
	$(call help_line,frey test,Run Frey Vitest)
	$(call help_line,frey install,Install Frey dependencies)
	$(call help_line,frey build,Build Frey for production)
	$(call help_line,frey lint,Run Frey ESLint)
	$(call help_line,frey format,Run Frey Prettier)
	$(call help_line,frey preview,Preview Frey production build)
	@printf "### n8n\n"
