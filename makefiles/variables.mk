...
VECTORDB_DIR ?= applications/VectorDB
QA_DIR ?= applications/beacon-qa
BEACON_TOOLS_DIR ?= applications/beacon-tools
...
VECTORDB_COMPOSE_FILE ?= $(VECTORDB_DIR)/docker-compose.yml
QA_COMPOSE_FILE ?= $(QA_DIR)/docker-compose.yml
BEACON_TOOLS_COMPOSE_FILE ?= $(BEACON_TOOLS_DIR)/docker-compose.yml
...
COMPOSE_VECTORDB := docker compose --env-file $(VECTORDB_DIR)/.env -f $(VECTORDB_COMPOSE_FILE)
COMPOSE_QA := docker compose -f $(QA_COMPOSE_FILE)
COMPOSE_BEACON_TOOLS := docker compose -f $(BEACON_TOOLS_COMPOSE_FILE)
...
VECTORDB_PSQL_IMAGE ?= postgres:18-alpine
VECTORDB_HOST ?= vectordb.beacon.famillelallier.net
VECTORDB_PUBLIC_PORT ?= 5433
VECTORDB_QUERY ?= SELECT 1;
