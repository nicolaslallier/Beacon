...
### 3. Docker Compose (Split Frontend/Backend)

The Library backend depends on shared services managed by infra
(`beacon-library-postgres`, `beacon-library-chromadb`, `beacon-library-ollama`,
`beacon-library-gotenberg`, `beacon-library-mailhog`, `beacon-library-mcp-vector`).
Start the infra stack (library profile) first:

```bash
docker compose -f ../../infra/docker-compose.yml --profile library up -d
```

Run the backend API stack:

```bash
docker compose -f docker-compose.backend.yml up -d
```

Run the frontend stack (Vite dev server + Nginx proxy):
