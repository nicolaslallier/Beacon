...
lint-shell:
	@if which shellcheck >/dev/null 2>&1; then \
	  ...
	  docker run --rm -v "$(PWD)/scripts:/code" koalaman/shellcheck:stable *.sh ; \
	fi

lint-nginx:
	@$(DOCKER) run --rm -v "$(PWD)/templates/nginx.conf.template:/etc/nginx/nginx.conf:ro" nginx:1.27-alpine nginx -t || true
